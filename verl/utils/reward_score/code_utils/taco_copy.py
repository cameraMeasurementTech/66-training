...
def execute_cb_code(method, inputs_list, outputs_list, timeout, early_stop=True, debug=True):
    ...
    for index, inputs in enumerate(inputs_list):
        ...
        try:
            ...
            #                         env=env,
            #                         check=False,
            #                         timeout=timeout + 5)  # 添加额外的超时保护

            command.extend(["/share/miniconda3/envs/verl_v0_szp/bin/python"  "-c", method])
            result = subprocess.run(command, 
...
def execute_std_code(method, synthesized_code, inputs_list, outputs_list, timeout, early_stop=False, debug=False):
    ...
    for i, inputs in enumerate(inputs_list):
        ...
        with tempfile.NamedTemporaryFile(mode='w+') as temp_input:
            ...
            stdout, stderr = "", ""
            try:
                # result = subprocess.run(['bash', '-c', 'ulimit -v 10485760; /share/miniconda3/envs/verl_v0_szp/bin/python ' + temp_program_path], 
                #                         stdin=temp_input,
                #                         stdout=subprocess.PIPE,
                #                         stderr=subprocess.PIPE,
                #                         preexec_fn=os.setsid,
                ...
                tmp_command.extend(["/share/miniconda3/envs/verl_v0_szp/bin/python",temp_program_path])
                ...
                # result = subprocess.run(['/share/miniconda3/envs/verl_v0_szp/bin/python', temp_program_path], input=inputs, text=True, capture_output=True, timeout=timeout)
                exec_code = 999
